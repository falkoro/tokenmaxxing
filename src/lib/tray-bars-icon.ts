...
export function makeTrayBarsSvg(args: {
  ...
  /**
   * Glyph color. Black on macOS (template icons are recolored by the menu
   * bar); white elsewhere, where the template flag is ignored and taskbars
   * are usually dark.
   */
  glyphColor?: string
}): string {
  const { bars, sizePx, style = "provider", percentText, providerIconUrl, glyphColor = "black" } = args
  ...
  if (style === "provider") {
    ...
    if (href.length > 0) {
      ...
      parts.push(
        `<circle cx="${cx}" cy="${cy}" r="${radius}" fill="none" stroke="${glyphColor}" stroke-width="${strokeW}" opacity="1" shape-rendering="geometricPrecision" />`
      )
    }
  } else if (style === "donut") {
    ...
    if (href.length > 0) {
      ...
      parts.push(
        `<circle cx="${fcx}" cy="${fcy}" r="${fallbackR}" fill="none" stroke="${glyphColor}" stroke-width="${fallbackSW}" opacity="1" shape-rendering="geometricPrecision" />`
      )
    }
    ...
    parts.push(
      `<circle cx="${cx}" cy="${cy}" r="${radius}" fill="none" stroke="${glyphColor}" stroke-width="${strokeW}" opacity="${BARS_TRACK_OPACITY}" shape-rendering="geometricPrecision" />`
    )
    ...
    if (typeof fraction === "number" && Number.isFinite(fraction) && fraction >= 0) {
      ...
      if (clamped > 0) {
        ...
        parts.push(
          `<circle cx="${cx}" cy="${cy}" r="${radius}" fill="none" stroke="${glyphColor}" stroke-width="${strokeW}" stroke-linecap="butt" stroke-dasharray="${dash} ${circumference}" transform="rotate(-90 ${cx} ${cy})" opacity="${BARS_FILL_OPACITY}" shape-rendering="geometricPrecision" />`
        )
      }
    }
  } else {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      parts.push(
        `<rect x="${x}" y="${y}" width="${trackW}" height="${trackH}" rx="${rx}" fill="${glyphColor}" opacity="${trackOpacity}" />`
      )
      ...
      if (typeof fraction === "number" && Number.isFinite(fraction) && fraction >= 0) {
        ...
        if (fillW > 0) {
          ...
          if (fillW >= trackW) {
            parts.push(
              `<rect x="${x}" y="${y}" width="${fillW}" height="${trackH}" rx="${rx}" fill="${glyphColor}" opacity="${fillOpacity}" />`
            )
          } else {
            ...
            parts.push(`<path d="${fillPath}" fill="${glyphColor}" opacity="${fillOpacity}" />`)
          }
        }
        ...
        if (fillW > 0 && remainderDrawW > 0 && dividerX !== null) {
          ...
          parts.push(`<path d="${remainderPath}" fill="${glyphColor}" opacity="${remainderOpacity}" />`)
        }
      }
    }
  }
  ...
  if (text) {
    parts.push(
      `<text x="${layout.textX}" y="${layout.textY}" fill="${glyphColor}" font-family="-apple-system,BlinkMacSystemFont,'SF Pro Text',sans-serif" font-size="${layout.fontSize}" font-weight="700" dominant-baseline="middle">${escapeXmlText(text)}</text>`
    )
  }
...
export async function renderTrayBarsIcon(args: {
  ...
  const { bars, sizePx, style = "provider", percentText, providerIconUrl, glyphColor } = args
  const text = normalizePercentText(percentText)
